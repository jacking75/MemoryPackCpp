...
> **이 문서는 2026-08-31부로 [`ROADMAP.md`](../../ROADMAP.md)와
> [`docs/dev/hardening-plan.md`](hardening-plan.md)로 대체되었다.** 아래 표의
> 상태(특히 "P1-2 단위 테스트 ✅ (59 checks)", "P1-3 CI ⬜")는 2026-06-23 시점
> 스냅샷이며 갱신되지 않는다 — 실제로는 체크 수가 훨씬 늘었고 CI는 의도적으로
> 두지 않기로 결정됐다(로컬 검증은 `tools/verify.ps1` 참고). 현재 상태가
> 필요하면 이 문서 대신 위 두 문서를 볼 것.
>
